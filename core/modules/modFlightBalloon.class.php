...
class modFlightBalloon extends DolibarrModules {
    ...
    function modFlightBalloon($DB) {
        ...
        $this->description = "Flotte du club";
        ...
        $this->requiredby = array('modFlightLog'); // List of modules id to disable if this one is disabled
        ...
        // Example:
        $this->rights[$r][0] = 9981;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'Ajouter un nouveau Ballon.'; // Permission label
        $this->rights[$r][3] = 1;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'bal';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $this->rights[$r][5] = 'access';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $r++;

        $this->rights[$r][0] = 9988;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'Ajouter un nouveau Ballon.'; // Permission label
        ...
        $this->rights[$r][0] = 9986;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'Permet de supprimer un ballon.'; // Permission label
        $this->rights[$r][3] = 0;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'bal';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $this->rights[$r][5] = 'del';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $r++;

        $this->rights[$r][0] = 9985;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'Editer un ballon.'; // Permission label
        $this->rights[$r][3] = 0;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'bal';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        ...
        $this->rights[$r][0] = 9984;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'ajouter un composant à un ballon'; // Permission label
        $this->rights[$r][3] = 0;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'piece';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $this->rights[$r][5] = 'add';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $r++;

        $this->rights[$r][0] = 9983;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'retirer une piece d\'un ballon'; // Permission label
        $this->rights[$r][3] = 0;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'piece';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $this->rights[$r][5] = 'del';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $r++;

        $this->rights[$r][0] = 9982;     // Permission id (must not be already used)
        $this->rights[$r][1] = 'editer une piece'; // Permission label
        $this->rights[$r][3] = 0;      // Permission by default for new user (0/1)
        $this->rights[$r][4] = 'piece';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $this->rights[$r][5] = 'edit';    // In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
        $r++;
        ...
        $this->menu[$r] = array('fk_menu' => 0, // Put 0 if this is a top menu
            ...
            'url' => '/flightBalloon/pages/balloons.php',
            'langs' => 'mylangfile', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 200,
            'enabled' => '1', // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled.
            'perms' => '$user->rights->flightBalloon->bal->access', // Use 'perms'=>'$user->rights->mymodule->level1->level2' if you want your menu with a permission rules
            'target' => '',
        ...
        $this->menu[$r] = array('fk_menu' => 'r=1', // Use r=value where r is index key used for the parent menu entry (higher parent must be a top menu entry)
            ...
            'titre' => 'Ajouter',
            'mainmenu' => 'flightBalloon',
            'url' => '/flightBalloon/pages/add.php',
            'langs' => 'mylangfile', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 202,
            'enabled' => '1', // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled.
            'perms' => '$user->rights->flightBalloon->bal->add', // Use 'perms'=>'$user->rights->mymodule->level1->level2' if you want your menu with a permission rules
            'target' => '',
            'user' => 2);    // 0=Menu for internal users, 1=external users, 2=both
        $r++;
        $this->menu[$r] = array('fk_menu' => 'r=1', // Use r=value where r is index key used for the parent menu entry (higher parent must be a top menu entry)
            ...
            'titre' => 'attribuer',
            'mainmenu' => 'flightBalloon',
            'url' => '/flightBalloon/pages/attribuer.php',
            'langs' => 'mylangfile', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 203,
            'enabled' => '1', // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled.
            'perms' => '$user->rights->flightBalloon->bal->add', // Use 'perms'=>'$user->rights->mymodule->level1->level2' if you want your menu with a permission rules
        ...
        $this->menu[$r] = array('fk_menu' => 'r=4', // Use r=value where r is index key used for the parent menu entry (higher parent must be a top menu entry)
            ...
            'titre' => 'Ajouter',
            'mainmenu' => 'flightBalloon',
            'url' => '/flightBalloon/pages/pieces/add.php',
            'langs' => 'mylangfile', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 205,
            'enabled' => '1', // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled.
            'perms' => '$user->rights->flightBalloon->piece->add', // Use 'perms'=>'$user->rights->mymodule->level1->level2' if you want your menu with a permission rules
            'target' => '',
            'user' => 2);    // 0=Menu for internal users, 1=external users, 2=both
        $r++;
