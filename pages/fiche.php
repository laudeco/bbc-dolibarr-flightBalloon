...
//declasser ballon
if ($_POST['action'] == 'classerconfirm' && $_POST['confirm'] == 'yes') {
    if ($_GET['bal']) {
        $tmp = New Bbc_ballons($db);
        $tmp->fetch($_GET['bal']);
        if (isset($tmp->id)) {

            $tmp->is_disable = !$tmp->is_disable;

            if ($tmp->update($user) > 0) {
                Header("Location: balloons.php");
            } else {
                $msg = '<div class="error">Erreur lors de la MAJ' . $bal->error . '</div>';
                $error++;
            }
        }
    }
...
if ($_POST['action'] == 'edit' && !$_POST['cancel']) {
    ...
    $bal->date = $dated;
    $bal->init_heure = $_POST['heures'] . ":" . $_POST['initmin'] . ":00";
    $bal->immat = $_POST['immat'];
    $bal->fk_responsable = $_POST['resp'];
    $bal->fk_co_responsable = $_POST['co_resp'];
...
if ($_GET["action"] == 'classer') {
    $html = New Form($db);
    $ret = $html->form_confirm("fiche.php?bal=" . $_GET['bal'], "Declasser Ballon", "Etes vous sure de vouloir declasser ce ballon?", 'classerconfirm','',"yes");
    if ($ret == 'html')
        print '<br>';
}
if ($msg) {
...
$coPilot = New User($db);
$coPilot->fetch($balloon->fk_co_responsable);


/** TAB * */
print '<div class="tabs">';
print '<a class="tabTitle"><img src="/theme/eldy/img/object_user.png" border="0" alt="" title=""> Fiche Ballon</a>'; //title
print '<a id="active" class="tab" href="fiche.php?bal=' . GETPOST('bal') . '">Fiche Ballon</a>';
print '<a class="tab" href="amp.php?bal=' . GETPOST('bal') . '">AMP</a>';
//print '<a class="tab" href="/categories/categorie.php?id=94&amp;type=3">Catégories</a>';
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && $user->rights->flightBalloon->bal->edit) {
    ...
    $dateSplited = explode(":",$balloon->init_heure);
    print '<input type="text" name="heures" value="'.$dateSplited[0].'"/>';
    print '<select name="initmin">';
    for($i = 1; $i <=59;$i++){
        print '<option value="'.$i.'"'.($dateSplited[1] == $i?'selected':'').'>'.$i.'</option>';
    }
    print '</select>';
//    print $html->select_date($balloon->init_heure, "init", 0, 1, 0, "heures", 0);
    print '</td></tr>';
    //Immatriculation
    print "<tr>";
    ...
    //CO responsable
    print "<tr>";
    print '<td width="25%" class="fieldrequired">Co-Responsable </td><td>';
    print $html->select_users($balloon->fk_co_responsable, 'co_resp', 1);
    print '</td></tr>';
    print '</table>';
    ...
    print '<tr><td>identifiant</td><td>' . $balloon->id . '</td></tr>';
    $dateSplited = explode(":",$balloon->init_heure);
    print '<tr><td>H init</td><td>' . $dateSplited[0].':'.$dateSplited[1]. '</td></tr>';
    ...
    print '<tr><td>Co-Responsable </td><td><a href="' . DOL_URL_ROOT . '/user/fiche.php?id=' . $balloon->fk_co_responsable . '">' . img_object($langs->trans("ShowUser"), "user") . $coPilot->getFullName($langs, 0, 0) . '</a></td></tr>';
...
if (!isset($_GET['action'])) {
    ...
    if ($user->rights->flightBalloon->bal->delete || $user->admin) {
        if($balloon->is_disable){
            print '<a class="butAction" href="fiche.php?action=classer&bal=' . $balloon->id . '">Re-voler</a>';
        }else{
            print '<a class="butAction" href="fiche.php?action=classer&bal=' . $balloon->id . '">Declasser</a>';
        }
    }

    //bouton modifier si on a le droit ou si c'est son vol
    if ($user->rights->flightBalloon->bal->edit || $user->admin ||$balloon->fk_responsable == $user->id || $balloon->fk_co_responsable == $user->id) {
