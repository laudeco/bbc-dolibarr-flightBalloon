...
if($resql){
	...
	if($num){
		...
		print '<td class="liste_titre" > Date dernier vol </td>';
		print '<td class="liste_titre"> marraine </td>';
		print '<td class="liste_titre"> responsable </td>';
		print '<td class="liste_titre"> Back Up </td>';
		...
		while($i <$num){
			...
			if($obj){
				...
				$co_responsable->fetch($obj->fk_co_responsable);
//				print '<td><a href="ficheBalloon.php?bal='.$obj->idBal.'">'.strtoupper($obj->ballon).'</a></td>';
				...
				print '<td><a href="'.DOL_URL_ROOT.'/user/fiche.php?id='.$obj->fk_co_responsable.'">'.img_object($langs->trans("ShowUser"),"user").' '.$co_responsable->getFullName($langs).'</a></td>';
				print '<td>'.  img_picto('statut', 'statut'.$obj->is_disable).'</td>';
			}
			print'</tr>';
			$i++;
		}
